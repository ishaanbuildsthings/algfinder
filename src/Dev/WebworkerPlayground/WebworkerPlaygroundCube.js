const { simplifyMove } = require('./webworkerPgAlgHandler');

// ! rotational symmetry utility____________________________
const key = {
  '⬜': 0,
  '🟩': 1,
  '🟥': 2,
...
// this function allows for the rotational symmetry encoding inside the hashing function
// eslint-disable-next-line no-unused-vars
function reorderWithAuxillaryMemory(matrix) {
  // console.log(`the input matrix is:`);
  // console.log(matrix);
  // console.log(JSON.stringify(matrix) === JSON.stringify(RThenX));

  if (JSON.stringify(matrix) === JSON.stringify(RThenX)) {
    // console.log('hit:')
    // console.log(`the output matrix is:`);
    // console.log(RThenXThenXprime);
    return RThenXThenXprime;
  }
  const orderedState = [null, null, null, null, null, null];
  for (const sublist of matrix) {
    const center = sublist[4];
    const position = key[center];
    orderedState[position] = sublist;
  }
  // console.log(`the output matrix is:`);
...
class Cube {
  ...
  // getReorderedState() {
  //   const state = [this.uFace, this.fFace, this.rFace, this.dFace, this.lFace, this.bFace];
  //   const reorderedState = reorderWithAuxillaryMemory(state);
  //   return reorderedState[0].join('') + reorderedState[1].join('') + reorderedState[2].join('') + reorderedState[3].join('') + reorderedState[4].join('') + reorderedState[5].join('');
  // }
  ...
  // only difference is this allows rotations at the beginning
  createAdjList() {
    const adjList = [];
    let simplifiedLastMove;
    if (this.movesApplied.length !== 0) {
