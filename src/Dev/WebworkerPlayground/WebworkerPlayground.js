...
while (depthOfNextQueuedCube < depth) {
  ...
  for (let scrambledAdjacentCube of scrambledAdjacencyList) {
    ...
    } else { // if scrambled state has been seen from scrambled end, just add it
    ...
    if (scrambledAdjacentCube.getReorderedState() in solvedHash) {
      console.log('This cube state has been reached from the solved end before! Intersection found.');
      console.log('Here are the ways we reached this state from the solved end:');
...
if (finalSolutions.size === 0) { // AND CUBE IS SOLVED
  console.log('no solutions found, this should post an empty solution if cube is solved')
} else {
