...
export default function processMoves(scramble) {
  ...
  for (let i = 0; i < scrambleNoSpaces.length; i += 1) {
    if (
      i === scrambleNoSpaces.length - 1 &&
      /[RUFLDBxyzEMSrufldb]/.test(scrambleNoSpaces[i])
    ) {
      // if at the last move and a move matches, add that and return the string
